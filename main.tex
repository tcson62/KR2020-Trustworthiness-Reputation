...
\begin{abstract}
The paper proposes a framework for answering the question of whether  statements 
made by an agent can be believed, in light of observations made over time. 
The basic components of the framework are a formalism for 
reasoning about actions, changes, and observations and a formalism for default reasoning. 
The paper describes a concrete system implemented using answer set programming for  
asserting the truthfulness of statements made by agents, starting
 from observations, knowledge about the actions of the agents, and a theory about the 
 ``normal'' behavior of agents. 
\textcolor{red}{[REMOVE sentence]} The paper also introduces the notions of \emph{supporters} and 
\emph{denials} of a statement, which can be used to guide 
the process of determining the truthfulness of statements. The paper presents an application of the 
proposed framework in the detection of
 man-in-the-middle attacks targeting computers and cyber-physical systems. 
Finally, the paper briefly 
discusses related work   and   possible extensions. 
\end{abstract}
...
}} reported by BBC and Sky News). Another example is the case of a hotel reviewer with the alias {\em ``AmishBoy''} who gave high ratings to different hotels in the same day \cite{MinnichCMLF15}---thus proving  that his statements are untrustworthy. These situations pose the problem of  \emph{understanding whether 
statements made by an agent can be considered truthful within a context composed of observations and
knowledge about default behavior. }
...
In this paper, we are interested in reasoning about the truthfulness of statements made by agents. We assume that we can observe the world as well as other agents' actions. The basis for our judgments will be composed of our observations, performed along
a linear time line, along with  our commonsense knowledge about agents' behavior and the world. We assume that observations are \emph{true} at the time they are made, and will stay true until additional pieces of information prove otherwise. Our judgments reflect what we believe. They might not correspond to the ground truth and could change over time. This is because we often have to make our judgment in presence of incomplete information. This makes reasoning about the truthfulness of statements made by agents \emph{non-monotonic}. Furthermore, our judgment against a statement is \emph{independent} of whether or not we trust the agent from whom the statement originated. As such, our study in this paper differs from the 
...
We later learn that John attends a private college, renowned for  its expensive tuitions (time $t_1$). Later on, we learn 
that John attends the college thanks to a  scholarship awarded  due to his financial hardship (time $t_2$).

This story spans over three time instances: $t_0$, $t_1$, and $t_2$. At each time instance, we learn (through observations) some facts or receive some statements, which  affect our beliefs about John's statement as follows:    

\begin{list}{$\bullet$}{\itemsep=0pt \topsep=0pt \parsep=0pt \leftmargin=10pt}  
\item Time $t_0$: John says that his family is poor (property $poor$ is true). It is likely that we would believe John---since
 we have no reasons to conclude otherwise. 

\item Time $t_1$:  We observe the fact that John attends an expensive college (property $in\_college$ is true). {Since students attending the college are \emph{normally} from rich families} (default $d_1$), this would lead us to  conclude that John has lied to us. We indicate that  the default $d_1$ is the reason to draw such conclusion. 

\item Time $t_2$: We observe the fact that John has a need-based scholarship   (property $has\_scholarship$ is true). {Since a student's hardship is \emph{usually} derived from the family's financial situation} (default $d_2$), this fact   allows us to withdraw the conclusion that John is a liar, made at  time instance $t_1$. It is still insufficient for us to conclude that  John's family is poor. 

The situation might be different if, for example, we have a preference among defaults. In this example, if we are inclined to believe in the conclusion of $d_2$ more than that of $d_1$, then we would believe that John's family is poor and thus restore our trust in John's 
original statement (i.e., truth of $poor$). 
% 
\end{list} 
\end{example}  
...
represent and reason about truthfulness of agent's statements; 
\item A concrete realization of the model using
Answer Set Programming; 
\item \textcolor{red}{[REMOVE]} The notion of a \emph{supporter} and the notion of a 
        \emph{denial} 
of a statement, that can be used for identifying 
the truthfulness of statements; and 
\item Application of the proposed framework to a sample problem in the
...
\section{A General Model for Reasoning about Truthfulness of Statements made by Agents} 

In this section, we propose a general framework for representing, and reasoning about, the truthfulness of (statements made by) agents\footnote{
   From now on, we will often use ``the truthfulness of agents'' interchangeably with  
   ``the truthfulness of statements made by agents.''
}. The
...
%
 \begin{itemize}
%
\item It is possible to  observe the properties of the world and the occurrences of the agents' actions    over time
...
\item We have commonsense knowledge about ``normal'' behavior (e.g., a person attending an expensive school normally comes from a rich family, a person obtaining need-based scholarship   usually comes from a poor family).
This knowledge is represented by a default theory with preferences $\textit{Def}$,  that enables reasoning about the state of the world and deriving conclusions whenever necessary. Let us denote with $\models_D$ the entailment relation defined over the default theory framework defining $\textit{Def}$.  
 
\end{itemize} 

The set of observations $O_w$ in Example~\ref{ex1} includes the observations such as `John comes from a poor family' at time point $t_0$, `John attends an expensive college' at time point $t_1$, and `John receives a need-based scholarship' at time point $t_2$. In this particular example we do not have any  action occurrences, i.e., $O_a = \emptyset$. Our default theory $D$ consists of  $d_1$ and $d_2$, which allow us to make conclusions regarding whether John comes from a rich family or not. 

\begin{figwindow}[1,r,%
        {\includegraphics[width=0.7\textwidth]{newstep.pdf}},%
        {Answering Question: $(O_a,O_w,Act,Def) \models p[t]$?\label{fig:schema} }]
Let us consider a theory $T = (O_a, O_w, Act, \textit{Def})$ and the associated entailment relations $\models_A$ and $\models_D$. We are interested in answering the question of whether a statement asserting that a proposition $p$ is true  at a certain time step $t$, denoted by $p[t]$, is true or not. Specifically, we would like to define the entailment relation $\models$ between $T$ and $p[t]$. Intuitively, this can be done in two steps (Figure~\ref{fig:schema}):
\end{figwindow}
...
\item Determine  whether $p$ is true  given  $\textit{Def}$ and $W[t]$ (using $\models_D$). 
\end{list} 
%
...
{\bf (b)} the set of ASP atoms $\{mem(l, name) \mid l \in body\}$; similar encodings are used for  \eqref{default} and \eqref{pref}. 
This method of encoding is similar to what used  in answer set planning and allows for the separation between  the encoding of the problem and the rules for reasoning with it.
